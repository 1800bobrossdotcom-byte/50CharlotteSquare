# Font licenses

Both typefaces are licensed under the SIL Open Font License, Version 1.1, which permits
bundling and self-hosting them with this website.

- **Bricolage Grotesque** — Copyright 2022 The Bricolage Grotesque Project Authors
  (https://github.com/ateliertriay/bricolage). License: `OFL-bricolage-grotesque.txt`
- **Manrope** — Copyright 2018 The Manrope Project Authors
  (https://github.com/sharanda/manrope). License: `OFL-manrope.txt`
- **Instrument Serif** — Copyright 2022 The Instrument Serif Project Authors
  (https://github.com/Instrument/instrument-serif). License: `OFL-instrument-serif.txt`
- **DM Sans** — Copyright 2014-2023 Indian Type Foundry (https://github.com/googlefonts/dm-fonts). License: `OFL-dm-sans.txt`
