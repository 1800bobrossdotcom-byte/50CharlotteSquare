...
**89 has two tenants.** Ugly Duck Coffee and Rosebud Retro share a turn-of-the-century house (Ugly Duck's own site gives 89 Charlotte Street, parking off Pitkin). They share one row, both names on the heading line beside the number. Two earlier treatments were rejected: a second name stacked below the first, which left it looking numberless, and a separate row for each, which printed 89 twice.
